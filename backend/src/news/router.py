...
from ..error import handle_api_exception
...
def fetch_news_with_upvote_details(db: Session = Depends(session_opener)):
    try:
        ...
        handle_api_exception(e, "fetching news articles", status.HTTP_500_INTERNAL_SERVER_ERROR, "An error occurred while fetching news articles. Please try again later.")
    ...
    try:
        ...
        for article in news:
            try:
                ...
                handle_api_exception(e, f"fetching upvote details for article {article.id}", status.HTTP_500_INTERNAL_SERVER_ERROR, "An error occurred while fetching upvote details for the article.")
            result.append(
                {
                    **article.__dict__,
                    "upvotes": upvotes,
        ...
        handle_api_exception(e, "fetching user-specific news articles", status.HTTP_500_INTERNAL_SERVER_ERROR, "An error occurred while fetching news articles from the database. Please try again later.")
    except Exception as e:
        handle_api_exception(e, "unexpected error", status.HTTP_500_INTERNAL_SERVER_ERROR, "An unexpected error occurred while fetching user-specific news. Please try again later.")
...
async def search_news_articles(request: PromptRequest):
    ...
    try:
        ...
        handle_api_exception(e, "extracting search keywords", status.HTTP_400_BAD_REQUEST, "Failed to extract search keywords from the prompt.")
    ...
    try:
        ...
        handle_api_exception(e, "sorting news articles", status.HTTP_500_INTERNAL_SERVER_ERROR, "An error occurred while sorting the news articles.")
...
async def news_summary(
    ...
    try:
        ...
        handle_api_exception(e, "generating news summary", status.HTTP_500_INTERNAL_SERVER_ERROR, "An error occurred while generating the news summary. Please try again later.")
